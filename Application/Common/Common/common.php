...
function get_sub_images($sub_img,$goods_id,$width,$height){
	//判断缩略图是否存在
        //本地存储
    if(strpos($sub_img['image_url'],'ftp://')===false){
        $path = "Public/upload/goods/thumb/$goods_id/";
        ...
        $rpath = "Public/upload/goods/thumb/$goods_id/";
        $path = "upload/goods/thumb/$goods_id/";
        $goods_thumb_name = "goods_sub_thumb_{$sub_img['img_id']}_{$width}_{$height}";
        //这个缩略图 已经生成过这个比例的图片就直接返回了
        $ftpConfig = C('UPLOAD_SITEIMG_FTP');
        $conn = ftp_connect($ftpConfig['host'], $ftpConfig['port'], 90);
        //登陆ftp
        if(!ftp_login($conn,$ftpConfig['username'],$ftpConfig['password'])) return '';
        $cotents = ftp_nlist($conn, $path);
        if($cotents===false||count($cotents)<1){//文件夹不存在
            //创建文件夹
            $re=mkdir_ftp($conn,$path);
        }
        $original_img = $sub_img['image_url']; //相对路径启用远端存储
        if (!ftp_size($conn,substr($sub_img['image_url'],strpos($sub_img['image_url'],'Uploads')))) return '';
        ftp_chdir($conn,$path);
        $remotepath=$path.$goods_thumb_name;//远端路径
        $image = new \Think\Image();
        $image->open($original_img);
        if(ftp_size($conn,$goods_thumb_name.'.jpg')>0)  return 'ftp://'.$ftpConfig['host'].':'.$ftpConfig['port'].'/'.$remotepath.'.jpg';
        if(ftp_size($conn,$goods_thumb_name.'.jpeg')>0) return 'ftp://'.$ftpConfig['host'].':'.$ftpConfig['port'].'/'.$remotepath.'.jpeg';
        if(ftp_size($conn,$goods_thumb_name.'.gif')>0)  return 'ftp://'.$ftpConfig['host'].':'.$ftpConfig['port'].'/'.$remotepath.'.gif';
        if(ftp_size($conn,$goods_thumb_name.'.png')>0)  return 'ftp://'.$ftpConfig['host'].':'.$ftpConfig['port'].'/'.$remotepath.'.png';
        $image = new \Think\Image();
        $image->open($original_img);
        $goods_thumb_name = $goods_thumb_name . '.' . $image->type();
        $remotepath=$path.$goods_thumb_name;//远端路径
        // 生成缩略图
        if (!is_dir($rpath))
            mkdir($rpath, 777, true);
        if (!file_exists($rpath . $goods_thumb_name)) {
            $image->thumb($width, $height, 2)->save($rpath . $goods_thumb_name, NULL, 100); //按照原图的比例生成一个最大为$width*$height的缩略图并保存
        }
        $localpath = $rpath . $goods_thumb_name;
        if (ftp_put($conn, $goods_thumb_name, $localpath, FTP_BINARY)) {
            $t = unlink($localpath);
        }
        ftp_close($conn);
 ...
 * Ftp创建文件夹
 */
function mkdir_ftp($conn,$path){
    $patha="";
    $ret=true;
    $dir=explode("/",$path);
    for($i=0;$i<count($dir);$i++){
        $patha.="/".$dir[$i];
        if(!@ftp_chdir($conn,$patha)){
            @ftp_chdir($conn,"/");
            if(!@ftp_mkdir($conn,$patha)){
                $ret=false;
                break;
            }
        }
